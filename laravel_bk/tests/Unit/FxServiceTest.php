...
class FxServiceTest extends TestCase
{
    ...
    public function search()
    {
        ...
        $result->comment = '';
        $tradeList = new TradeList($result);
        $tradeRepositoryMock = \Mockery::mock(TradeRepository::class);
        $tradeRepositoryMock->shouldReceive('search')->with('2020-07-06', '2020-07-06')->andReturn([$tradeList]);
        ...
        $trade = $results[0];
        $this->assertEquals('2020/07/06', $trade->getTradingDate());
        $this->assertEquals('Bid', $trade->getTradeType());
    }
}
